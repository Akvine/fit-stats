...
public class TelegramBotConfig {
    ...
    public TelegramBot telegramBot(MessageHandler messageHandler, DefaultBotOptions defaultBotOptions) throws TelegramApiException {
        ...
        TelegramBot bot;
        if (BOT_TYPE_LONGPOOLING.equals(propertyParseService.get(botType))) {
            bot = new TelegramLongPoolingBot(defaultBotOptions,
                    propertyParseService.get(botToken),
                    propertyParseService.get(botUsername),
                    messageHandler);
    ...
    public MessageProcessor messageProcessor(TelegramBot telegramBot) {
        String type = propertyParseService.get(botType);
        if (BOT_TYPE_LONGPOOLING.equals(type)) {
            return new TelegramLongPoolingMessageProcessor((TelegramLongPollingBot) telegramBot, propertyParseService);
        } else {
            throw new TelegramConfigurationException("Telegram bot with type = [" + type + "] is not supported!");
        }
    }
    ...
    private void setWebhook(String botToken) {
        ...
        if (!getWebhookInfoResponse.isOk() || !propertyParseService.get(botWebhookPath).equals(getWebhookInfoResponse.getResult().getUrl())) {
            SetWebhookResponse setWebhookResponse = sendSetWebhookRequest(botToken, propertyParseService.get(botWebhookPath));
    ...
    private SetWebhookResponse sendSetWebhookRequest(String botToken, String botPath) {
        ...
        request.setAllowedUpdates(propertyParseService.parseToList(allowedUpdates));
