...
@Component
public class AlcoholProcessor implements MacronutrientProcessor {
    @Override
    public List<Double> extract(List<DietRecordEntity> records) {
        return records.stream().map(DietRecordEntity::getAlcohol).collect(Collectors.toList());
    }

    @Override
    public Macronutrient getType() {
        return Macronutrient.ALCOHOL;
    }
}
